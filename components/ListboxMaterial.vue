        ...
        >
          <ListboxOptions class="absolute z-10 mt-2 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
            <ListboxOption
              ...
              v-for="spool in spoolsWithUniqueMaterials"
              :key="spool.material.id"
              :value="spool"
              as="template"
            >
              <li :class="[
...
const spoolsWithUniqueMaterials = computed(
  () => filamentSpoolStore.getSpoolsUniqueMaterials
);
