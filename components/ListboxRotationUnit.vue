        ...
        >
          <ListboxOptions class="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
            <ListboxOption
              ...
              v-for="rotationUnit in rotationUnits"
              :key="rotationUnit"
              :value="rotationUnit"
              as="template"
            >
...
const selectedUnit = ref<string>(rotationUnits[0]);

watch(selectedUnit, (value) => {
  globalsStore.setRotationUnit(
    RotationUnit[value as keyof typeof RotationUnit]
  );
});
</script>