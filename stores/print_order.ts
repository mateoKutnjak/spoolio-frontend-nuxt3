...
import { IAddressBilling, IAddressShipping, IAttachmentFile, IAttachmentImage, IPricing, IPrintOrder, IPrintOrderUnit, IPrintOrderUnitPlanes, IPrintingMethod } from '~~/constants/data';
...
export function adjustPrice(item: IPrintOrderUnit) {
    if (!item.pricing_list) {
        console.error(`Cannot calculate adjusted price. Pricing list is not defined for localUrl=${item.localUrl}`);
        return Number.POSITIVE_INFINITY;
    }

    const pricing = item.pricing_list.find((el) => el.q_from <= item.quantity);
    if (!pricing){
        console.error(`Cannot calculate adjusted price. Pricing list quantity is not found`);
        return Number.POSITIVE_INFINITY;
    }

    const unit_price = Math.round(100*(pricing.unit_price + pricing.prep_cost/item.quantity))/100;

    console.log("Unit price: %f", unit_price);
...
export const usePrintOrderStore = defineStore('print-order', {
    ...
    actions: {
                ...
                if (value) {
                    ...
                    const body = JSON.stringify(this.units.filter(el => el.estimated_time).map(el => {
                        return {
                            ...
                            material_id: el.spool.material.id,
                            estimated_time: el.estimated_time,
                            model_dimensions: el.model_dimensions,
                            length_unit: el.length_unit
                        }
                    }))
            ...
            watch(data, value => {
                ...
                switch (parsedData.type) {
                    ...
                    case 'data':
                        ...
                        const pricing_list = parsedData.data?.pricing_list as IPricing[];

                        if (!estimated_time || !estimated_price || !pricing_list) {
                        ...
                        break;
                    case 'error':
                        const errorMessage = parsedData.error as string;
                        console.error(`Websocket error: ${errorMessage}`)

                        dialogStore.open('DialogError', { message: errorMessage.replaceAll(' ', "_").toLowerCase() });
        ...
        async add3dModelFile(file: File, onFinishedCallback: (localUrl: string) => void, invalidFormatMessage: string) {
            ...
            const { model_volume, model_dimensions, optimal_rotation, model_rotation, border_planes, border_groups, unique_planes } = await preprocess3dObject(localUrl);

            const unit_planes = <IPrintOrderUnitPlanes>{
                border_planes: border_planes,
                border_groups: border_groups,
                unique_planes: unique_planes
            };

            console.log("Border planes: %o", border_planes);
            console.log("Border groups: %o", border_groups);
            console.log("Unique planes: %o", unique_planes);
            ...
            create3dObjectScreenshot(localUrl, filamentSpoolStore.getAll[0].color.value, 400, 400, (blob, simplifiedModelBlob) => {
                ...
                const unit = <IPrintOrderUnit>{
                    ...
                    model_xyz_rotation: vector3ToString(model_rotation),
